...
// TODO: check below class
public class MovieIdIndexPage implements Page {
    ...
    public MovieIdIndexPage(int pageId) {
        this.pageId = pageId;
        keys = new ArrayList<>();
    ...
    public void setIsLeaf(boolean leaf) {
        isLeaf = leaf;
        order = isLeaf ? MOVIE_ID_LEAF_NODE_ORDER : MOVIE_ID_NON_LEAF_NODE_ORDER;
    }
    ...
    public byte[] getByteArray() {
        ...
        for (byte[] key : keys) {
            System.arraycopy(key, 0, pageArray, offset, MOVIE_ID_SIZE);
            offset += MOVIE_ID_SIZE;
        }
    ...
    public void setByteArray(byte[] arr) {
        ...
        for (int i = 0; i < numKeys; i++) {
            byte[] key = new byte[MOVIE_ID_SIZE];
            System.arraycopy(pageArray, offset, key, 0, MOVIE_ID_SIZE);
            keys.add(key);
            offset += MOVIE_ID_SIZE;
        }
