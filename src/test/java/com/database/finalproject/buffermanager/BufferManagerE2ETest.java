...
class BufferManagerE2ETest {
    ...
    // MovieRecord movieRecord1 = new MovieRecord("tt0000001".getBytes(), "Movie
    // One".getBytes());
    ...
    // assertEquals("MovieRecord{movieId=tt0000001, title=Movie One}",
    // reloadedPage.getRecord(0).toString());
    ...
    // MovieRecord movieRecord1 = new MovieRecord("tt0000001".getBytes(), "Movie
    // One".getBytes());
    // MovieRecord movieRecord2 = new MovieRecord("tt0000002".getBytes(), "Movie
    // Two".getBytes());
    // page.insertRecord(movieRecord1);
    ...
    // // Query the rows
    // assertEquals("MovieRecord{movieId=tt0000001, title=Movie One}",
    // page.getRecord(0).toString());
    // assertEquals("MovieRecord{movieId=tt0000002, title=Movie Two}",
    // page.getRecord(1).toString());
    ...
    // MovieRecord movieRecord3 = new MovieRecord("tt0000003".getBytes(), "Movie
    // Three".getBytes());
    // MovieRecord movieRecord4 = new MovieRecord("tt0000004".getBytes(), "Movie
    // Four".getBytes());
    ...
    // assertEquals("MovieRecord{movieId=tt0000002, title=Movie Two}",
    // page.getRecord(1).toString());
    // assertEquals("MovieRecord{movieId=tt0000003, title=Movie Three}",
    // page.getRecord(2).toString());
    // assertEquals("MovieRecord{movieId=tt0000004, title=Movie Four}",
    // page.getRecord(3).toString());
    ...
    // reloadedPage.getRecord(0).toString());
    // assertEquals("MovieRecord{movieId=tt0000002, title=Movie Two}",
    // reloadedPage.getRecord(1).toString());
    // bufferManager.unpinPage(reloadedPage.getPid());
    ...
    // MovieDataPage page = (MovieDataPage) bufferManager.getPage(120000); //
    // Non-existent page ID
    // assertNull(page, "Fetching a non-existent page should return null");
    // }

    // @Test // test that the dataset is getting loaded properly and pages are being
    // fetched
    // // correctly
    ...
    // assertTrue(inBufferTime < avgTime, "MovieDataPage 1 should be in the buffer "
    // + avgTime + " " + inBufferTime);
    ...
    // MovieRecord("tt9999999".getBytes(StandardCharsets.UTF_8),
    ...
    // DatabaseCatalog catalog = new
    // DatabaseCatalog("src/main/resources/static/database_catalog.txt");
    // try (RandomAccessFile raf = new
    // RandomAccessFile(catalog.getCatalog(WORKED_ON_DATA_PAGE_INDEX).get("filename"),
    // "r")) {
    // byte[] pageData = new byte[PAGE_SIZE];
    // raf.seek((long) (newPageId) * PAGE_SIZE);
    // raf.readFully(pageData);
    // assertNotNull(pageData, "MovieDataPage should be written to the binary
    // file");
    // }
    // catch (IOException e){
